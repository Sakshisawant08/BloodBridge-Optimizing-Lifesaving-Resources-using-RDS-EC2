...
# ============================================
# Architecture Flow:
# User → Route 53 → CloudFront → ALB → EC2 (Flask App) → RDS (MySQL)
...
def login():
    ...
    if request.method == 'POST':
        ...
        def verify_password(stored_password, provided_password):
            ...
            if stored_password.startswith('pbkdf2:') or stored_password.startswith('$2'):
                try:
...
def register():
    ...
    if request.method == 'POST':
        ...
        """, (name, email, hashed_password, phone, blood_group, age, gender, 
              address, city, last_donation))
...
def update_request_status(request_id):
    ...
    cur = mysql.connection.cursor()
    cur.execute("UPDATE blood_requests SET status = %s, updated_at = NOW() WHERE id = %s", 
                (status, request_id))
    mysql.connection.commit()
    cur.close()
...
def donor_dashboard():
    ...
    cur.execute("SELECT * FROM blood_drives WHERE date >= CURDATE() ORDER BY date LIMIT 5")
...
def hospital_dashboard():
    ...
    cur.execute("SELECT * FROM donors WHERE status = 'active' ORDER BY created_at DESC LIMIT 10")
...
def request_blood():
    ...
    if request.method == 'POST':
        ...
        """, (session['user_id'], blood_group, quantity, priority, patient_name, 
              reason, required_by))
        mysql.connection.commit()
        cur.close()
...
def init_db():
    ...
    # Update all users with proper hash
    cur.execute("UPDATE users SET password = %s WHERE email = 'admin@bloodbridge.com'", (hashed,))
    cur.execute("UPDATE users SET password = %s WHERE email = 'citygen@hospital.com'", (hashed,))
    cur.execute("UPDATE users SET password = %s WHERE email = 'stmary@hospital.com'", (hashed,))
    cur.execute("UPDATE users SET password = %s WHERE email = 'metro@hospital.com'", (hashed,))
    cur.execute("UPDATE users SET password = %s WHERE email = 'central@bloodbank.com'", (hashed,))

    # Update all donors with proper hash
    cur.execute("UPDATE donors SET password = %s WHERE email = 'john@email.com'", (hashed,))
    cur.execute("UPDATE donors SET password = %s WHERE email = 'sarah@email.com'", (hashed,))
    cur.execute("UPDATE donors SET password = %s WHERE email = 'michael@email.com'", (hashed,))
    cur.execute("UPDATE donors SET password = %s WHERE email = 'emily@email.com'", (hashed,))
    cur.execute("UPDATE donors SET password = %s WHERE email = 'robert@email.com'", (hashed,))
    cur.execute("UPDATE donors SET password = %s WHERE email = 'lisa@email.com'", (hashed,))
    cur.execute("UPDATE donors SET password = %s WHERE email = 'david@email.com'", (hashed,))
    cur.execute("UPDATE donors SET password = %s WHERE email = 'jennifer@email.com'", (hashed,))
